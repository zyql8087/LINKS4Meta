...
def _require_tensorflow():
    if tf is None:
        raise ImportError('tensorflow is required for GPU solver functions.')
...
def solve(path,x0,G,motor,fixed_nodes,theta):
    ...
    for step in path:
        ...
        cosphi = (l_ij **2 + G[i,k]**2 - G[j,k]**2) / denom
        if cosphi >= -1.0 and cosphi <= 1.0:
            s = np.sign((x0[i,1]-x0[k,1])*(x0[i,0]-x0[j,0]) - (x0[i,1]-x0[j,1])*(x0[i,0]-x0[k,0]))
            phi = s * np.math.acos(np.clip(cosphi, -1.0, 1.0))
            R = np.array([[np.cos(phi), -np.sin(phi)],
                          [np.sin(phi), np.cos(phi)]])
            scaled_ij = (x[j]-x[i]) / max(l_ij, eps) * G[i,k]
...
def solve_rev_vectorized(path,x0,G,motor,fixed_nodes,thetas):
    ...
    for step in path:
        ...
        denom = 2 * l_ij * G[i,k]
        invalid = denom <= eps
        numerator = (l_ij ** 2 + G[i,k]**2 - G[j,k]**2)

        cosphi = np.zeros_like(l_ij)
        valid_denom = np.logical_not(invalid)
        cosphi[valid_denom] = numerator[valid_denom] / denom[valid_denom]
        cosphi[invalid] = np.nan

        invalid = np.logical_or(invalid, np.logical_or(cosphi < -1.0, cosphi > 1.0))
        invalid = np.logical_or(invalid, np.isnan(cosphi))
        state += invalid
        ...
        safe_cosphi = np.clip(np.nan_to_num(cosphi, nan=1.0), -1.0, 1.0)
        phi = s * np.arccos(safe_cosphi)
        ...
        safe_l_ij = np.maximum(l_ij, eps)
        scaled_ij = (x[j]-x[i])/np.expand_dims(safe_l_ij,-1) * G[i,k]
...
def simulate_mechanism(M,n=200,motor=None):
    ...
    if motor is None:
        if A.shape[0] > 2 and A[0,2] and node_type[0] == 1 and node_type[2] == 0:
            motor = [0,2]
        else:
            motor = [0,1]
    ...
    x_sol, valid, _ = solve_rev_vectorized(path, x0, get_G(x0), motor, fixed_nodes, np.linspace(0,2*np.pi,n))

    if np.isnan(x_sol).sum() != 0 or np.sum(valid) != len(valid):
        return None
...
def draw_mechanism(A,x0,fixed_nodes,motor, highlight=100, solve=True, thetas = np.linspace(0,np.pi*2,200), def_alpha = 1.0, h_alfa =1.0, h_c = "#f15a24"):
    ...
    fixed_marker = p if p is not None else 's'
    ...
    for i in range(N):
        if i in fixed_nodes:
            if i == highlight:
                plt.scatter(x[i,0],x[i,1],color=h_c,s=700,zorder=10,marker=fixed_marker)
            else:
                plt.scatter(x[i,0],x[i,1],color="#1a1a1a",s=700,zorder=10,marker=fixed_marker)
        else:
            if i == highlight:
    ...
    if solve:
        path, path_found = find_path(A,motor,fixed_nodes)
        if not path_found or len(path) == 0:
            plt.text(0.5, 0.5, 'Path Not Found', color='red', horizontalalignment='center', verticalalignment='center')
        else:
            G = get_G(x0)
