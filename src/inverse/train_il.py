...
def extract_expert_paths(pkl_path: str, output_path: str):
    ...
    for sample_idx, sample in enumerate(tqdm(raw_data, desc="Extracting IL expert paths")):
        try:
            ...
            # gen_info missing 鈫?old pkl format
            if gen_info is None:
                errors += 1
                continue

            u, v, w = gen_info['u'], gen_info['v'], gen_info['w']
            n1, n2 = gen_info['n1'], gen_info['n2']

            # 鈹€鈹€ 閲嶅缓 4 鏉嗗熀纭€鍥?(鍘绘帀 n1, n2 鑺傜偣鍙婂叾鎵€鏈夎繛杈? 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
def _build_geo_conditions(x_enc, action_topo, z_c, nodes_per_graph=4):
    """
    ...
    u_f = x_2d[torch.arange(B), action_topo[:, 0]]               # (B, hidden)
...
def _local_undirected_edges(edge_index, nodes_per_graph=4):
    edges = set()
    for u, v in edge_index.detach().cpu().numpy().T.tolist():
        lu = int(u % nodes_per_graph)
        lv = int(v % nodes_per_graph)
        if lu == lv:
            continue
        a, b = sorted((lu, lv))
        edges.add((a, b))
...
def compute_geometry_prior_regularizer(pred_geo, base_data, action_topo, cfg, nodes_per_graph=4):
    ...
    base_pos = base_data.pos.view(batch_size, nodes_per_graph, 2)
    batch_idx = torch.arange(batch_size, device=pred_geo.device)

    u_idx = action_topo[:, 0]
    v_idx = action_topo[:, 1]
    w_idx = action_topo[:, 2]
    u_pos = base_pos[batch_idx, u_idx]
    v_pos = base_pos[batch_idx, v_idx]
    w_pos = base_pos[batch_idx, w_idx]
    n1 = pred_geo[:, :2]
    n2 = pred_geo[:, 2:]
    ...
    node_mask = torch.ones(batch_size, nodes_per_graph, dtype=torch.bool, device=pred_geo.device)
    node_mask[batch_idx, u_idx] = False
    node_mask[batch_idx, v_idx] = False
    node_mask[batch_idx, w_idx] = False
    dist_n1 = torch.norm(n1.unsqueeze(1) - base_pos, dim=-1)
    dist_n2 = torch.norm(n2.unsqueeze(1) - base_pos, dim=-1)
    node_penalty = (
        F.softplus(node_margin - dist_n1)[node_mask].mean()
        + F.softplus(node_margin - dist_n2)[node_mask].mean()
    ) if node_mask.any() else pred_geo.new_tensor(0.0)

    edge_penalties = []
    local_edges = _local_undirected_edges(base_data.edge_index, nodes_per_graph=nodes_per_graph)
    candidate_segments = [
        ((u_idx, 'n1'), u_pos, n1),
        ((v_idx, 'n1'), v_pos, n1),
        (('n1', 'n2'), n1, n2),
    ...
    for seg_nodes, seg_a, seg_b in candidate_segments:
        seg_anchor_ids = [node for node in seg_nodes if isinstance(node, torch.Tensor)]
        for e_u, e_v in local_edges:
            shares = torch.zeros(batch_size, dtype=torch.bool, device=pred_geo.device)
            for anchor in seg_anchor_ids:
                shares |= (anchor == e_u) | (anchor == e_v)
            if torch.all(shares):
                continue
            edge_a = base_pos[:, e_u]
            edge_b = base_pos[:, e_v]
            clearance = _segment_clearance_distance(seg_a, seg_b, edge_a, edge_b)
            penalty = F.softplus(node_margin - clearance)
            if (~shares).any():
                edge_penalties.append(penalty[~shares].mean())

    new_pairs = [
        (0, 3),
        (1, 3),
    ]
    for idx_a, idx_b in new_pairs:
        _, seg_a0, seg_a1 = candidate_segments[idx_a]
        _, seg_b0, seg_b1 = candidate_segments[idx_b]
        clearance = _segment_clearance_distance(seg_a0, seg_a1, seg_b0, seg_b1)
    ...
    return short_edge_penalty + node_penalty + edge_penalty
...
def eval_il_epoch(policy, curve_encoder, dataloader, device, cfg):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            cond = _build_geo_conditions(x_enc, action_topo, z_c)
