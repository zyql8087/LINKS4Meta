"""将原始机构样本（dict格式）转换为 PyG Data 对象，用于运动学曲线预测。"""
...
# 机构族名称与ID互查
FAMILY_TO_ID = {"6bar": 0, "7bar": 1, "8bar": 2, "9bar": 3}
...
def _semantic_role_matrix(num_nodes: int, analysis: Mapping[str, object]) -> np.ndarray:
    """构建 (num_nodes, 4) 的语义角色 one-hot 矩阵，列对应 hip/knee/ankle/foot。"""
...
def build_step_context(sample: Mapping[str, object]) -> np.ndarray:
    """构建 [step_count, aux_steps, semantic_steps] 三维特征向量。"""
...
def _padded_geometry_descriptor(sample: Mapping[str, object]) -> np.ndarray:
    """提取几何描述子并零填充/截断至 MAX_GEOMETRY_DESCRIPTOR_DIM 维。"""
    geometry = np.asarray(
...
def sample_to_pyg_data(sample: Mapping[str, object], sample_id: int) -> Data:
    ...
    edge_index = np.array(np.where(A)).T.T  # PyG格式 (2, num_edges)
    ...
    return Data(
        ...
        mask_hip=torch.tensor(semantic_roles[:, 0] > 0.5, dtype=torch.bool),
        mask_knee=torch.tensor(semantic_roles[:, 1] > 0.5, dtype=torch.bool),
        mask_ankle=torch.tensor(semantic_roles[:, 2] > 0.5, dtype=torch.bool),
        mask_foot=torch.tensor(semantic_roles[:, 3] > 0.5, dtype=torch.bool),
